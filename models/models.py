...
def model_init(config: argparse.Namespace, device):
    ...
    if config['use_new_f']:
        ...
        for n in range(config['n_flows_F']):
            for i in range(3):
                F_flows['MNet' + str(n) + '_' + str(i)] = F_MulNet(2, config['emb_dim'], config['n_neurons']).to(device)
                F_flows['MNet' + str(n) + '_' + str(i)].apply(init_weights)
                F_flows['ANet' + str(n) + '_' + str(i)] = F_AddNet(2, config['emb_dim'], config['n_neurons']).to(device)
