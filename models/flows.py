...
def F_flow(x, e, F_flows, n_flows_F):
    ...
    for n in range(n_flows_F):
        for k in range(3):
            h1, h2 = x[:, :2], x[:, 2, None]
            M = F_flows['MNet' + str(n) + str(k)](h1, e)
            A = F_flows['ANet' + str(n) + str(k)](h1, e)
            h2 = h2 * torch.exp(M) + A
...
def F_inv_flow(z, e, F_flows, n_flows_F):
    for n in range(n_flows_F-1, -1, -1):
        for k in range(2, -1, -1):
            h1, h2 = z[:, 1:], z[:, 0, None]
            M_inv = torch.exp(-F_flows['MNet' + str(n) + str(k)](h1, e))
            A = F_flows['ANet' + str(n) + str(k)](h1, e)
            h2 = (h2 - A) * M_inv
...
def G_flow(w, G_flows, n_flows_G, emb_dim):
    ...
    for n in range(n_flows_G):
        for k in range(2):
            ...
            M = G_flows['MNet' + str(n) + str(k)](h1)
            A = G_flows['ANet' + str(n) + str(k)](h1)
            h2 = h2 * torch.exp(M) + A
            ldetJ += torch.sum(M, dim=1).view(-1, 1)
