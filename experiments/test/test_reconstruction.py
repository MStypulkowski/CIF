...
def main(config: argparse.Namespace):
    ...
    if config['trained_recon']:
        ...
        w4recon.load_state_dict(torch.load(config['load_models_dir'] + r'embs.pth'))
        w4recon.eval()
        with torch.no_grad():
            w = w4recon()
    else:
    ...
    # ):
    mean = torch.from_numpy(np.load(config["resume_dataset_mean"])).float().to(device)
    std = torch.from_numpy(np.load(config["resume_dataset_std"])).float().to(device)
    ...
    torch.save(samples, config['load_models_dir'] + name)
