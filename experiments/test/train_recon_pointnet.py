...
def main(config: argparse.Namespace):
    ...
    n_samples = cloud_pointflow.all_points.shape[0]
    cloud_size = 2048
    ...
    samples = (
        torch.cat(samples, dim=0).reshape((n_samples, cloud_size, 3))
        # .to(device)
    )

    torch.save(
        samples, config["load_models_dir"] + "reconstructions_train.pth"
    )
    torch.save(
        ref_samples, config["load_models_dir"] + "references_train.pth"
    )
    ...
    # samples = torch.load(config['load_models_dir'] + 'reconstructions_train.pth').to(device)
