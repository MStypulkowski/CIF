...
def main(config: argparse.Namespace):
    ...
    with torch.no_grad():
        init_emb = G_flow(w, G_flows, config['n_flows_G'], config['emb_dim'])

    embs4recon = Embeddings4Recon(config, init_emb).to(device)
    ...
    for i in range(config['n_epochs']):
        ...
        if (i + 1) % 50 == 0:
            ...
            torch.save(embs4recon.state_dict(), config['embs_dir'] + r'embs.pth')
